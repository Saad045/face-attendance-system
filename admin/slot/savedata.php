...
$sql_check = "SELECT * FROM slot WHERE slot_no = '$s_no' AND slot_time = '$tstart'";
...
if (mysqli_num_rows($result_check) > 0) {
    ...
    echo "Slot already exists in the database.";
    exit();
}
elseif (mysqli_num_rows($result_check1) > 0) {
    // If data already exists, show error message and exit script
    echo "Start time already exists in the database.";
    exit();
}
elseif (mysqli_num_rows($result_check2) > 0) {
    // If data already exists, show error message and exit script
    echo "End time already exists in the database.";
    exit();
}
